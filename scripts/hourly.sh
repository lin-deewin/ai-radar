...
# export AI_RADAR_WXPUSHER_APP_TOKEN=AT_xxxxxxxxx
...
if [ -x "$FISH_BIN" ]; then
    for var in AI_RADAR_WXPUSHER_APP_TOKEN AI_RADAR_WXPUSHER_UID \
...
# 方式 C: 从 ~/.bashrc 读取（若有）
if [ -z "${AI_RADAR_WXPUSHER_APP_TOKEN:-}" ] && [ -z "${AI_RADAR_PUSHPLUS_TOKEN:-}" ] \
   && [ -z "${AI_RADAR_SERVERCHAN_KEY:-}" ] && [ -z "${AI_RADAR_WECOM_BOT_KEY:-}" ] \
   && [ -f "$HOME/.bashrc" ]; then
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] hourly.sh start; wxpusher_token=${AI_RADAR_WXPUSHER_APP_TOKEN:+set}; uid=${AI_RADAR_WXPUSHER_UID:+set}"
